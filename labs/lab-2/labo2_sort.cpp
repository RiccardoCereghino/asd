...
void qs(vector<int>&v, int start, int end){
  if(start<end){
    int pivot_index = partition_in_place(v, start, end);
    qs(v,start,pivot_index-1);
    qs(v,pivot_index+1,end);
  }
}

int partition_in_place(vector<int>& v, int l, int r){
  int p = v[l];
  int i = l + 1;
  for(int j=i; j<=r; ++j){
    if(v[j]<p){
...
void quickSortRandom(vector<int>& v)
{
  ...
  qs_rand(v,0,v.size()-1);
}

void qs_rand(vector<int>& v, int start, int end){
  if(start<end){
    int pivot_index = partition_in_place_random(v, start, end);
...
int partition_in_place_random(vector<int>& v, int l, int r){
  int p = l+rand() % (r-l+1);
  swap(v[p],v[l]);
  int i = l+1;
  for(int j=i; j<=r; ++j){
